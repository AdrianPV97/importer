...
SELECT CLIENTE_ID FROM CLAVES_CLIENTES WHERE CLAVE_CLIENTE = :v_clave_cliente INTO :CLIENTE_ID    ;
...
BEGIN
    SELECT CLIENTE_ID FROM CLIENTES WHERE CLIENTE_ID IN (1, 2, 3/*lista de clientes de credito*/) INTO :CREDITO_VALIDACION;
...
SELECT MONEDA_ID, COND_PAGO_ID, VENDEDOR_ID, RECEPTOR_CFD FROM CLIENTES WHERE CLIENTE_ID = :CLIENTE_ID INTO :MONEDA_ID, :COND_PAGO_ID, :VENDEDOR_ID, :RECEPTOR_CFD;
...
VALUES (
:FACTURA_ID, 'F', :V_FOLIO, :SUCURSAL_ID, :FECHA_SRVR, :HORA_SRVR, :V_CLAVE_CLIENTE, :CLIENTE_ID, :DIR_CLI_ID, :DIR_CLI_ID,
...
VALUES(
    -1, :V_UUID, :FOLIOS_FISCALES_ID, :FOLIO_FISCAL , :fecha_srvr, 'VE', :FACTURA_ID , :V_XML)     ;


-- EXECUTE PROCEDURE GENERA_DOCTO_CC_VE (:FACTURA_ID) RETURNING_VALUES :DOCTO_CC_INS;
...
-- :V_IMPORTE_NETO, 'Aplicacion de ANticipo' || :v_descripcion, current_date, :V_FOLIO_AP_ANT, :CLIENTE_ID) RETURNING_VALUES :APLICACION;
